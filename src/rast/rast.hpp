...
struct bgfx_impl {
  ...
  void view_set_transform(bgfx::ViewId p_id, const m::mat<ff32, 4, 4> &p_view,
  ...
  void set_transform(const m::mat<ff32, 4, 4> &p_transform) {
    ...
    proxy().for_each_renderpass([&](RenderPassProxy &p_render_pass) {
      ...
      // color clear
      {
        ...
        if (l_clear_state.m_flags.m_depth) {
          ...
          l_depth_view.for_each<ff32>(
              [&](ff32 &p_pixel) { p_pixel = l_clear_state.m_depth; });
        }
      }
...
inline void setViewTransform(ViewId _id, const void *_view, const void *_proj) {
  s_bgfx_impl.view_set_transform(_id, *(const m::mat<ff32, 4, 4> *)_view,
                                 *(const m::mat<ff32, 4, 4> *)_proj);
};

inline uint32_t setTransform(const void *_mtx, uint16_t _num) {
  s_bgfx_impl.set_transform(*(const m::mat<ff32, 4, 4> *)_mtx);
  return -1;
};
